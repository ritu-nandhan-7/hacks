...
const ITEMS: { id: Screen; label: string; icon: React.ReactNode }[] = [
  { id: "minimal-home",    label: "Home",    icon: <Home size={16} /> },
  { id: "minimal-quests",  label: "Quests",  icon: <Zap size={16} /> },
  { id: "minimal-bets",    label: "Bets",    icon: <Target size={16} /> },
  { id: "minimal-profile", label: "Profile", icon: <User size={16} /> },
];

export default function MinimalNav({ screen, setScreen }: { screen: Screen; setScreen: (s: Screen) => void }) {
  return (
    <nav style={navWrap}>
      {ITEMS.map(item => (
        <NavItem
